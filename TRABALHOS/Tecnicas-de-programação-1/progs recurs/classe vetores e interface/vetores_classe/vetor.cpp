...
vetor::vetor(int n)
{
    this->n = n; // capacidade maxima de elementos que podem ser aLocados
    vet = new int[n];
    qtd = 0;
}
...
{    
    ...
    if(x->qtd > y->qtd)
        ...
        for(i = 0 ; i < x->qtd ; i++)
        {
            ...
            if(x->vet[i] != y->vet[k])
                return false; // foi encontrado um elemento de A que nao esta em B
        }
...
int vetor::BUSCABINARIA(vetor* x,int pi,int pf,int elemento)
{
    if(pi == pf)
        return pf; // retorna posicao do elemento que esta sendo procurado

    int meio = (pi+pf)/2;

    if(elemento > x->vet[meio])
        return BUSCABINARIA(x,meio+1,pf,elemento);

    return BUSCABINARIA(x,pi,meio,elemento);
}
