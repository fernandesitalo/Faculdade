...
IndiceAl::IndiceAl()
{
    ...
    {
        INDICE.open("Alugueis.dat",ios_base::out );
        INDICE.close();
...
void IndiceAl::InserirOrdenadamenteAluguel(INDALUGA A)
{
    ...
    for(j = qtd-1 ; j >= 0 && A.CodigoDoAluguel < VetAlugados[j].CodigoDoAluguel ;j--)
    {
        VetAlugados[j+1] = VetAlugados[j];
    }
    VetAlugados[j+1] = A;
    VetAlugados[j+1].PosicaoNoArquivo = qtd;
...
bool IndiceAl::VerificarUnicidadeCodigo(long long cod)
{
    ...
    while(pi != pf)
    {
        ...
        if(cod > VetAlugados[meio].CodigoDoAluguel)
            pi = meio + 1;

        else  pf = meio;
    }
    if(cod == VetAlugados[pi].CodigoDoAluguel && !VetAlugados[pi].getLixo())
        return false;
...
int IndiceAl::PosicaoNoArquivo(long long cod)
{
    ...
    return VetAlugados[pi].PosicaoNoArquivo;
...
{
  if(qtd)
  return  VetAlugados[qtd-1].CodigoDoAluguel + 1;

  return VetAlugados[qtd].CodigoDoAluguel;
...
long long IndiceAl::getCodigoDoAluguel(int posicao)
{
    ...
    return VetAlugados[posicao].CodigoDoAluguel;
}

void IndiceAl::setCodigoDoAluguel(long long A)
{
    VetAlugados[qtd].CodigoDoAluguel = A;
}
...
{
    VetAlugados[BuscaNoIndice(codigo)].PosicaoNoArquivo = NovaPosicao;
}
...
int IndiceAl::BuscaNoIndice(long long codigo)
{
    ...
    while(pi != pf)
    {
        ...
        if(codigo > VetAlugados[meio].CodigoDoAluguel)
            pi = meio + 1;
